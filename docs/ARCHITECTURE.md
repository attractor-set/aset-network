...
Profile availability, transport, cryptographic verification, remote recognition and network observation do **not** activate a profile. For activation, the binding is projected directly into a Seed `ResolutionBinding`: target Context → `context_id`, target state root → `state_root`, profile digest → `question_digest`, target policy epoch → `policy_epoch`, and declared Seed scope → `scope`. `target_context_id` is constrained to the intersection of the Network and pinned Seed identifier domains, so every schema-valid profile binding is type-compatible with that projection. A fresh Seed `resolution_id` is then used. Applicability is derived only when that exact target-local Seed resolution evaluates `ALLOW`. It is not ambient state: a different Seed `state_root` requires a different exact binding/resolution unless the parent Context itself defines a stable state-root semantics that remains the exact Seed binding. The Network profile does not redefine that semantics. There is therefore no normative `active_profiles` registry and no second profile decision algebra. A changed definition has a different digest and requires a new exact binding; an earlier `ALLOW` does not carry forward.

Profiles may strengthen the parent contract but may not weaken it or supersede Seed. This is a normative refinement obligation, not an automatic proof status. `refinement_evidence_digest` is optional; when present it only binds an external assurance artifact, and `MECHANICALLY_PROVED` may be claimed only from independently verified evidence carrying that status. The optional dynamic-profile conformance kit checks RFC 8785/SHA-256 content addressing, Seed projection closure, exact-binding applicability and fail-closed negative cases. The Network core deliberately defines no universal profile dependency resolver, precedence system, composition algebra, negotiation protocol or runtime plugin lifecycle. A composed profile is responsible for its own constraints and refinement evidence.
