...
def validate(model):
    if model.get('version')!=EXPECTED_VERSION: raise SystemExit('ERROR: unsupported extension version')
    if model.get('status')!=EXPECTED_STATUS: raise SystemExit('ERROR: unsupported extension status')
    if model.get('transition_kinds')!=EXPECTED_TRANSITIONS: raise SystemExit('ERROR: unsupported transition catalogue')
    if model.get('state_partition',{}).get('semantic_state_fields')!=EXPECTED_FIELDS: raise SystemExit('ERROR: unsupported semantic-state partition')
    if model.get('state_partition',{}).get('evidence_history_fields')!=['history']: raise SystemExit('ERROR: unsupported history partition')
...
def main():
    ...
    model=json.loads(MODEL_PATH.read_text()); validate(model); expected=render(model)
    if args.check:
        if not OUTPUT_PATH.exists() or OUTPUT_PATH.read_text()!=expected: raise SystemExit('NETWORK_CANON_PROJECTION_CHECK=FAIL')
        ...
        print('NETWORK_CANON_PROJECTION_CHECK=PASS'); return 0
