...
class SeedBinding:
    ...
    assurance_bases: dict[str, tuple[str, str]]
...
def parse_seed_binding(path: Path = BINDING) -> SeedBinding:
    ...
    require(set(companions) == {"ENGLISH", "PYTHON"}, "Seed companion base surface mismatch")
    return SeedBinding(
...
def check_seed_extension(seed_root: Path) -> dict[str, object]:
    ...
    operational_path, operational_word = binding.assurance_bases["OPERATIONAL"]
    relational_path, relational_operator = binding.assurance_bases["RELATIONAL"]
    causal_path, causal_transition = binding.assurance_bases["CAUSAL"]

    seed_forth = (seed_root / operational_path).read_text(encoding="utf-8")
    require(
        re.search(
            rf"(?m)^:\s+{re.escape(operational_word)}\s+.*?UNKNOWN\?\s+OBSERVE\s*;\s*$",
            seed_forth,
        )
        is not None,
        "exact Seed operational OBSERVE-UNKNOWN boundary unavailable",
    )
    network_forth = (ROOT / "network/alpha4/operational/components.forth").read_text(
        encoding="utf-8"
    )
    require(
        "ADMIT-FRESH" in network_forth and "ADMIT-REPLAY" in network_forth,
        "Network accepted operational branches missing",
    )
    require(
        "LOCAL-ALLOW!" not in network_forth and "LOCAL-BLOCK!" not in network_forth,
        "Network operational layer acquired Seed recognition operation",
    )

    seed_relational = (seed_root / relational_path).read_text(encoding="utf-8")
    require(
        re.search(
            rf"(?m)^{re.escape(relational_operator)}\(s, t, e\)\s*==",
            seed_relational,
        )
        is not None,
        "exact Seed relational ObserveUnknown operator unavailable",
    )
    network_relational = (ROOT / "network/alpha4/formal/NetworkRelations.tla").read_text(
        encoding="utf-8"
    )
    require(
        (
            'SeedProjectionRecognition(result) == IF AcceptedResult(result) THEN "UNKNOWN" '
            'ELSE "NOT_APPLICABLE"'
        )
        in network_relational,
        "Network relational Seed projection drift",
    )
    require(
        "SeedProjectionEffectPermitted(result) == FALSE" in network_relational,
        "Network relational effect boundary drift",
    )

    seed_causal = (seed_root / causal_path).read_text(encoding="utf-8")
    seed_block = _transition_block(seed_causal, causal_transition)
    require(
        "FROM U" in seed_block and "TO U" in seed_block,
        "Seed causal OBSERVE-UNKNOWN no longer preserves UNKNOWN",
    )
    require(
        "EFFECT ADD_EVIDENCE" in seed_block,
        "Seed causal OBSERVE-UNKNOWN evidence effect missing",
    )
    network_causal = (ROOT / "network/alpha4/causal/components.petri").read_text(encoding="utf-8")
    for name in ("ADMIT-FRESH", "ADMIT-REPLAY"):
        block = _transition_block(network_causal, name)
        require("OUTPUT ACCEPTED TRUE" in block, f"{name}: accepted marker missing")
        require(
            "OUTPUT SEED_RECOGNITION UNKNOWN" in block,
            f"{name}: Seed recognition projection drift",
        )
        require("OUTPUT SEED_EFFECT FALSE" in block, f"{name}: Seed effect projection drift")
    rejected = _transition_block(network_causal, "REJECT-CONFLICT")
    require("OUTPUT ACCEPTED FALSE" in rejected, "REJECT-CONFLICT: rejection marker missing")
    require(
        "OUTPUT SEED_RECOGNITION NOT_APPLICABLE" in rejected,
        "REJECT-CONFLICT: rejected branch claimed Seed recognition",
    )
    require(
        "OUTPUT SEED_EFFECT FALSE" in rejected,
        "REJECT-CONFLICT: rejected branch acquired Seed effect",
    )

    manifest = _lines(NETWORK)
    required_manifest = {
    ...
    require(
        ...
        "Network representation-wise Seed extension bindings missing",
    )

    return {
        "document_type": "aset-network-seed-extension-assurance",
        ...
        "network_semantic_delta": "NETWORK_OWNED_ONLY",
...
def main() -> int:
    ...
    try:
        ...
        print("ALPHA4_NETWORK_SEED_OPERATIONAL_EXTENSION=PASS")
        print("ALPHA4_NETWORK_SEED_RELATIONAL_EXTENSION=PASS")
        print("ALPHA4_NETWORK_SEED_CAUSAL_EXTENSION=PASS")
        print("ALPHA4_NETWORK_SEED_EXTENSION_BINDINGS=3/3 PASS")
        print(f"ALPHA4_NETWORK_SEED_REDEFINITION={evidence['seed_redefinition']}")
        if args.seed_profiles_root is not None:
            check_seed_companion_bases(args.seed_profiles_root)
            print("ALPHA4_NETWORK_SEED_ENGLISH_COMPANION_BASE=EXACT")
            print("ALPHA4_NETWORK_SEED_PYTHON_COMPANION_BASE=EXACT")
        print("ALPHA4_NETWORK_SEED_EXTENSION_ASSURANCE=PASS")
        return 0
    except (OSError, UnicodeError, ValueError, SeedExtensionError) as error:
        print(f"ALPHA4_NETWORK_SEED_EXTENSION_ERROR={error}")
        print("ALPHA4_NETWORK_SEED_EXTENSION_ASSURANCE=FAIL")
        return 1
