...
def main() -> int:
    ...
    for module in MODULES:
        ...
        if result.returncode:
            print(f"ALPHA4_NETWORK_TLAPS_MODULE={module.name} FAIL")
            return result.returncode
        matches = re.findall(r"All ([0-9]+) obligations? proved\.", result.stdout)
        if not matches:
            print(f"ALPHA4_NETWORK_TLAPS_MODULE={module.name} SUMMARY_MISSING")
            return 1
        count = int(matches[-1])
        total += count
        print(f"ALPHA4_NETWORK_TLAPS_MODULE={module.name} OBLIGATIONS={count} PASS")
    print(f"ALPHA4_NETWORK_TLAPS_OBLIGATIONS={total} PASS")
